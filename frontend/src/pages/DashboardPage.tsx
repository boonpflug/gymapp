  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mt-5">
        <StatCard title={t('dashboard.facilities')} value={dash?.totalFacilities ?? '—'} accent="brand" icon="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
        <StatCard title={t('dashboard.newMembersMonth')} value={dash?.totalNewMembersThisMonth ?? '—'} accent="emerald" icon="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" />
        <StatCard title={t('dashboard.classesToday')} value={todayClasses.filter(c => !c.cancelled).length} accent="blue" icon="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
        {/* Recent Check-ins */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <h2 className="text-base font-semibold text-gray-900 mb-4">{t('dashboard.recentCheckins')}</h2>
          {recentCheckins.length === 0 ? (
            <p className="text-gray-400 text-sm">{t('dashboard.noRecentCheckins')}</p>
          ) : (
              ...
              {recentCheckins.map(c => (
                <div key={c.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-900">{c.memberName ?? t('dashboard.unknown')}</p>
        ...
        {/* Today's Classes */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <h2 className="text-base font-semibold text-gray-900 mb-4">{t('dashboard.todaysClasses')}</h2>
          {todayClasses.length === 0 ? (
            <p className="text-gray-400 text-sm">{t('dashboard.noClassesToday')}</p>
          ) : (
            <div className="space-y-3">
              {todayClasses.filter(c => !c.cancelled).map(c => (
                <div key={c.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-gray-400">{c.bookedCount}/{c.capacity} {t('dashboard.booked')}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {dash && dash.facilitySummaries && dash.facilitySummaries.length > 1 && (
        <div className="mt-8">
          <h2 className="text-base font-semibold text-gray-900 mb-4">{t('dashboard.facilityOverview')}</h2>
            ...
            {dash.facilitySummaries.map(s => (
              <div key={s.facility.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div><p className="text-gray-400 text-xs">{t('dashboard.members')}</p><p className="font-semibold text-gray-900">{s.activeMembers}</p></div>
                  <div><p className="text-gray-400 text-xs">{t('dashboard.checkins')}</p><p className="font-semibold text-gray-900">{s.checkInsToday}</p></div>
                  <div><p className="text-gray-400 text-xs">{t('dashboard.occupancy')}</p><p className="font-semibold text-gray-900">{s.currentOccupancy}{s.facility.maxOccupancy ? `/${s.facility.maxOccupancy}` : ''}</p></div>
                  <div><p className="text-gray-400 text-xs">{t('dashboard.revenue')}</p><p className="font-semibold text-emerald-600">€{Number(s.revenueThisMonth ?? 0).toFixed(0)}</p></div>
                </div>
              </div>
            ))}
          </div>
